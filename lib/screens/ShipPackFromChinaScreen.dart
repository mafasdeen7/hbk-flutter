...
class ShipPackFromChinaScreen extends StatefulWidget {
  static const routeName = "/ship-pack-china";

  @override
  _ShipPackFromChinaScreenState createState() =>
      _ShipPackFromChinaScreenState();
}

class _ShipPackFromChinaScreenState extends State<ShipPackFromChinaScreen> {
  ...
  _submitSupplierEmail() async {
    ...
    bool sendMail = await Provider.of<SendEmailProvider>(context, listen: false)
        .sendMail('china', supplierEmailCnt.text, token);
  ...
  Widget build(BuildContext context) {
    ...
    _supplierLinkCnt.text = "http://supplier.hbkglobaltrading.com";

    return Scaffold(
      appBar: AppBar(
        title: Text('Ship a package from China'),
      ),
      body: ListView(
        children: <Widget>[
          Container(
