...
class PaySupplierScreen extends StatefulWidget {
  static const routeName = "/pay-supplier";

  @override
  _PaySupplierScreenState createState() => _PaySupplierScreenState();
}
...
class _PaySupplierScreenState extends State<PaySupplierScreen> {
  ...
  TextEditingController _depositSlip = TextEditingController(
      text:
          'https://hbkglobaltrading.com/wp-content/uploads/gravity_forms/2-90e800ac5ee9d5eef7127d99a79dffc5/2020/03/Screen-Shot-2020-03-04-at-9.46.32-AM.jpg');
  String base64Image="";
  ...
  void showWidget(){
    setState(() {
      Navigator.pushNamed(context, PaySupplierNextScreen.routeName);
    });
  }
...
//https://www.coderzheaven.com/2019/04/30/upload-image-in-flutter-using-php/
  ...
  void didChangeDependencies() async {
    ...
    if (!_loadedInitData) {
      ...
      _pesoValue = await Provider.of<PesoValueProvider>(context).getPesoValue();
      setState(() {
        _loading = false;
      });
  ...
  Widget build(BuildContext context) {
    ...
    double multiply = 0;
    int addition = 0;
    _loading ? addition = 0 : addition = _pesoValue.addition;
    _loading ? multiply = 0 : multiply = _pesoValue.multiply;
    _needToPayCnt.text = "₱ " + addition.toString();
                ...
                children: <Widget>[
                  Card(
                    ...
                    child: Container(
                      ...
                      child: getCalculator(addition, multiply),
                    ),
                  ),
                  agreeButton()
                ],
